...
// STARS
add_action( 'woocommerce_after_shop_loop_item_title', 'add_custom_rating_imgs', 5 );

function add_custom_rating_imgs() {
    global $product;
    // Is there reviews?
    if ( $product->get_review_count() > 0 ) {
        ...
        for ($i = 1; $i <= 5; $i++) {
            if ($i <= $average_rating) {
                $output .= '<img src="' . get_template_directory_uri() . '/resources/images/full_star.png" alt="full star">';
            } else {
                // HALF STAR
                if ($i - 0.5 <= $average_rating) {
                    $output .= '<img src="' . get_template_directory_uri() . '/resources/images/half_star.png" alt="half star">';
                } else {
                    $output .= '<img src="' . get_template_directory_uri() . '/resources/images/empty_star.png" alt="empty star">';
                }
            }
        }
        $output .= '</div>';
